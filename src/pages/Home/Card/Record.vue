    ...
    <Card :title="$t('dialog.record.content_0')" :sub_title="$t('dialog.record.content_1')" icon="icon-1">
      <div slot="content">
        <div class="content">
          <div class="block">
            <div class="left">
              ...
              <div class="row">
                ...
                <div class="value">{{subCoin | floor}}</div> <!--自動參與-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_4')}}:</div>
                <div class="value">{{r5 | floor}}</div><!--最大分發-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_5')}}:</div>
                <div class="value">{{r6 | floor}}</div> <!--可取分發-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_6')}}:</div>
                <div class="value">{{ethDone | floor}}</div> <!--已取獎勵-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_35')}}:</div>
                <div class="value">{{sonCount }}</div> <!--合作夥伴-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_39')}}:</div>
                <div class="value">{{sonTotalAmount1 | floor }}</div> <!--直推業績-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_37')}}:</div>
                <div class="value">{{address | short_address}}</div> <!--錢包地址-->
              </div>
            </div>
            <div class="right">
              <div class="row">
              ...
              <div class="row">
                ...
                <div class="value">{{r3 | floor}}</div> <!--超級節點獎勵-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_9')}}:</div>
                <div class="value">{{r2 | floor}}</div> <!--節點獎勵-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_10')}}:</div>
                <div class="value">{{r1 | floor}}</div> <!--鏈接獎勵-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_11')}}:</div>
                <div class="value">{{r7 | floor}}</div>  <!--累計已提現-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_36')}}:</div>
                <div class="value">{{sonNodeCount }}</div> <!--節點夥伴-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_40')}}:</div>
                <div class="value">{{sonTotalAmount15 | floor }}</div> <!--團隊業績-->
              </div>
              <div class="row">
                <div class="label">{{$t('dialog.record.content_38')}}:</div>
                <div class="value">{{usdtBalance | floor}}</div> <!--TRC20 USDT-->
              </div>
            </div>
          </div>
          <div class="block">
            <div class="count-down-box">
              <div class="count-down-box-bg" :style="'width:' + (1 - count_down/(10*3600*24*1000)) * 100 +'%'"></div>
          ...
          <div v-if="!distributionDisable" class="btn none" :class="{disable:distributionDisable}"  @click="distributionReward">{{$t('dialog.record.content_13')}}</div>
          <div v-else class="btn none" :class="{disable:distributionDisable}" >{{$t('dialog.record.content_13')}}</div>
          <div class="block">
            <van-tabs type="card" class="tabs" background="#1e2234" color="#ed1b26" title-active-color="#fff" title-inactive-color="#fff">
              <van-tab :title="$t('dialog.record.content_14')">
                ...
                <table>
                  ...
                  <tr class="item" v-for="(item, index) in table1_list" :key="index">
                    ...
                    <td class="reward">{{item.amount | floor}}</td>
                  </tr>
                </table>
                </van-list>
              </van-tab>
...
import {getLogList_LogAddrAmount_List, getLogList_logProfit_List, getLogList_loglink_List} from '@/utils/Invite.js'
...
export default {
  ...
  computed: {
    distributionDisable: function() {
      return this.quitted > 0 ?true:false
    },
    is_over: function(){
    ...
    count_down: function() {
      const left = parseInt(this.distributeLastTime * 1000) + 10 * 24 * 3600 * 1000 - Date.now()
    ...
    level: function(){
      if(this.sonNodeCount >= 15){
        return this.$t('dialog.record.content_32')
      }else if(this.sonCount >= 15){
    ...
    ...mapState({
      ...
      investmentPre: state => formatWei(state.wallet.investmentPre),
      ...
      usdtBalance: state => formatWei(state.wallet.usdtBalance),
      ...
      quitted: state => formatWei(state.wallet.quitted),
      sonTotalAmount1: state => formatWei(state.wallet.sonTotalAmount1),
      sonTotalAmount15: state => formatWei(state.wallet.sonTotalAmount15),
    })
  },
  methods: {
    distributionReward,
