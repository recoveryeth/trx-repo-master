<script src="../../../../../truffle/MetaCoin/truffle-config.js"></script>
<template>
  <div id="box">
    <Card
          ...
          <div class="block">
            ...
            <div class="block-summary">
              <div class="block-summary-item row">
                <div class="amount">{{distributePool | floor}} USDT </div>
                <div class="label">{{$t('dialog.box.content_8')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{reserve | floor}} USDT</div>
                <div class="label">{{$t('dialog.box.content_9')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{profitPool | floor}} USDT</div>
                <div class="label">{{$t('dialog.box.content_10')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{devFund | floor}} USDT</div>
                <div class="label">{{$t('dialog.box.content_11')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{top3Pool | floor}} USDT</div>
                <div class="label">{{$t('dialog.box.content_12')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{boxPool | floor}} USDT</div>
                <div class="label">{{$t('dialog.box.content_13')}}</div>
              </div>
              <div class="block-summary-item">
                <div class="amount">{{insuranceFund | floor}} USDT</div>
...
import {getLogList_LogAddrAmount_Box} from '@/utils/Invite.js'
...
export default {
  ...
  mounted() {
    this.timer = setInterval(() => {
      if(this.address != null){
        ...
        getLogList_LogAddrAmount_Box(no).then((list) =>{
  ...
  methods: {
    ...
    boxeward(rank) { //计算应得宝箱奖励
      ...
      rank +=1;
      if (rank>=1 && rank<=3){
        return current_investment*6/100 *25/100/3;
      }
      else if(rank>=4 && rank<=10){
        return current_investment*6/100 *25/100 /7;
      }
      else if(rank>=11 && rank<=80){
        return current_investment*6/100 *25/100 /70;
      }
      else if(rank>=81 && rank<=580){
        return current_investment*6/100 *25/100 /500;
      }
      return 0;
    },
