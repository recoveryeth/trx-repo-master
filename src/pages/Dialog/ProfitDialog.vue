        ...
        <template v-if="list.length > 0">
          ...
          <div class="items">
            <div class="item" v-for="(item,index) in list" :key="index">
              <p class="row0">
                <span class="left">{{item.profit | floor}}USDT</span>
                <span class="middle"></span>
                <span class="right">
...
import { getLogList_Profit_List } from '@/utils/Invite.js'
  ...
  methods: {
    getTxUrl: function(tx) {
      if(this.$i18n.locale == "zh-TW") {
        return (BUILD_TYPE == 'PROD' ? 'https://cn.etherscan.com/tx/' : 'https://shasta.tronscan.org/#/transaction/') + tx
      } else {
        return (BUILD_TYPE == 'PROD' ? 'https://tronscan.io/#/transaction/' : 'https://shasta.tronscan.org/#/transaction/') + tx
      }
    },
    open: function() {
      this.$refs.dialog.open()
      getLogList_Profit_List(this.address).then(data => {
        this.list = data
