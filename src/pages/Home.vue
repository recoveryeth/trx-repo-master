  ...
  <div id="home">
    <div class="dashboard">
      <div class="graph">
        <div class="list">
          <div class="item">
            <div class="amount">
              ...
              <span>{{parseFloat(totalInvestment) * 0.80 + parseFloat(ethTax) | floor(0)}}</span>
            </div>
            <div class="label">
              {{$t('home.content_1')}}
            </div>
          ...
          <div class="item">
            <div class="amount">
              ...
              <span>{{parseFloat(totalInvestment) * 0.06 | floor(0)}}</span>
            </div>
            <div class="label">
              {{$t('home.content_5')}} 6%
            </div>
        ...
        <div id="container">
          ...
          <div class="data">
            ...
            <div class="label">{{$t('home.content_7')}}(USDT)</div>
          </div>
        </div>
        <div class="list">
          <div class="item">
          ...
          <div class="item">
            <div class="amount">
              <span>{{parseFloat(totalInvestment) * 0.01 | floor(0)}}</span>
      ...
      <div class="coutdown" >
        ...
        <div class="time" v-else-if="boxExpire>0">
          <van-count-down :time="boxExpire" />
          <div class="tip_boxexpire_coutdown" >{{$t('home.content_4')}}   </div>
        </div>
        <div class="time" v-else-if="investment>0&&boxRestartExpire>0">
          <div class="tip_restart_coutdown" >{{$t('home.content_8')}}</div>
          <van-count-down :time="boxRestartExpire" />
        </div>
        <div class="time" v-else-if="investment>0">
          <div class="tip_restart" >{{$t('home.content_9')}}</div>
        </div>
      </div>
    </div>
    <Join class="join" />
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        boxRestartExpire: state => (parseInt(state.wallet.boxExpire)*1000 > Date.now() ? parseInt(state.wallet.boxExpire)*1000 - Date.now() : 0)>0?0:((parseInt(state.wallet.boxExpire)*1000)+86400000)-Date.now(),
